...
### 修复
- 🐛 **双向流请求超时**：`StreamConnectionManager` 去掉对 `requestTimeout` 的错误 ×1000，与配置毫秒一致
- 🐛 **断连时尽快结束等待**：流结束/重连前对 `pendingRequests` 执行 `completeExceptionally`，避免 `sendRequest` 在 `get()` 上卡到超时

### 变更
- ⚡ 心跳线程池下限 `2 → 4`（`StreamBasedServiceCenterClient`）

## [2.0.5] - 2026-03-24

### 修复
- 🐛 **服务端重启客户端重连问题修复**
  - 修复 `StreamConnectionManager` 在服务端正常关闭连接时（`onCompleted`）不触发重连的问题
  - 服务端重启场景下现在会自动触发重连机制，确保客户端能够恢复连接
  - 修复心跳任务可能重复启动的问题，在启动新任务前会先取消旧任务
  - 优化重连逻辑，避免并发场景下的潜在问题

### 变更
- ⚡ **日志信息国际化**
  - `StreamConnectionManager` 所有日志信息调整为英文
  - 提升代码专业性和国际化标准
  - 保持日志格式统一，便于问题排查和监控
...
### 修复
- 🐛 **心跳携带完整服务信息**
  - 修复 `StreamBasedServiceCenterClient` 心跳仅传 nodeId 的问题
  - 心跳请求现在携带 namespaceId、groupName、serviceName 及完整节点信息，与 `ServiceRegistryManager` 保持一致
  - 网络重连后服务端可能已剔除不健康节点，携带 service 信息便于服务端恢复/更新节点
- 🐛 **心跳返回错误时打印 error 信息**
  - 心跳失败时使用 `logger.error` 输出 nodeId 和错误信息，便于排查问题

## [2.0.3] - 2026-03-03

### 修复
- 🐛 **双向流重连支持无限重试**
  - 修复 `StreamConnectionManager` 重连逻辑硬编码最多 5 次的问题
  - 使用 `config.getMaxReconnectAttempts()` 配置参数，与 `ConfigCenterManager` 保持一致
  - 当 `maxReconnectAttempts < 0` 时支持无限重试
...
| 2.0.3 | 2026-03-03 | 双向流重连支持无限重试 |
| 2.0.1 | 2026-01-29 | 修复重连时 nodeId 变更问题 |
...
- API 兼容；`sendRequest` 超时现为配置毫秒本意；断连时进行中请求会更快失败
- 更新依赖即可

### 从 2.0.4 升级到 2.0.5

- **完全兼容**：原有 API 无需修改
- **问题修复**：服务端重启时客户端能够正常重连，心跳任务不会重复启动
- **日志变更**：`StreamConnectionManager` 日志改为英文，如有日志监控需要调整关键字匹配
- **升级步骤**：更新依赖版本即可
